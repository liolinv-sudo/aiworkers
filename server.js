...
app.get("/api/agents/:id/outputs/:outputId/pdf", (req, res) => {
  ...
  const filename =
    (output.title || "alster").replace(/[^a-z0-9åäöÅÄÖ]+/gi, "_").slice(0, 60) + ".pdf";
  ...
  // Omslagssida
  doc.fontSize(24).text(output.title, { align: "center" });
  doc.moveDown();
  ...
  if (output.isBook && output.chapters?.length) {
    ...
    doc.addPage();
    doc.fontSize(15).text("Föreslagna marknadsplatser");
    doc.moveDown();
    ...
    doc.addPage();
    doc.fontSize(11).text(output.body || output.preview, { lineGap: 3 });
  }
