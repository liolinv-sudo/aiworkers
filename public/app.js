...
function renderOutputsList(agent) {
  ...
  outputsList.querySelectorAll(".btn-translate").forEach((btn) => {
    btn.addEventListener("click", () => translateOutput(agent.id, btn.dataset.outputId, btn));
  });

  outputsList.querySelectorAll(".btn-download-en-txt").forEach((btn) => {
    btn.addEventListener("click", () => {
      const output = agent.outputs.find((o) => o.id === btn.dataset.outputId);
      if (output) downloadOutputEn(agent, output);
    });
  });
}
...
function renderOutputItem(o) {
  ...
  const translateBtn = hasEn
    ? `<a class="btn btn-tiny btn-download" href="/api/agents/${outputsTargetId}/outputs/${o.id}/pdf?lang=en">⬇ .pdf (EN)</a>
       <button type="button" class="btn btn-tiny btn-download-en-txt" data-output-id="${o.id}">⬇ .txt (EN)</button>`
    : `<button type="button" class="btn btn-tiny btn-translate" data-output-id="${o.id}">🇬🇧 Översätt till engelska</button>`;
